    ...
    def oo_render(self, context=None):
        ...
        if len(self.oo_subreports)>0 or (self.output_format != self.source_format):
            ...
            try:
                ...
                os.system("taskkill /im soffice.bin /f")
                self.oo_render(context)

            #
            # Create a temporary (input for OpenOffice) file
            #
            dummy_fd1, temp_file_name = tempfile.mkstemp(suffix=".%s" % self.source_format, prefix='openerp_oot_i_')
            temp_file = open(temp_file_name, 'wb')
            try:
                #
                ...
                if self.output_format=='doc':
                    oohelper.replace_text_with_page_break(document)
                #
                ...
                if len(self.oo_subreports)>0:
                    ...
                    for subreport in self.oo_subreports:
                        ...
                        try:
                            ...
                            self.log("deleting subreport file failed "+subreport)
...
class OOHelper():
    ...
    OOPATHS = [ '/usr/bin/soffice', '/usr/lib64/ooo-2.0/program/soffice', '/opt/openoffice.org3/program/soffice', 'D:\\Program Files\\OpenOffice.org 3\\program\\soffice', ]
    ...
    def __init__(self, cPort='8100', autostart_openoffice=True, logger=None):
        """
    ...
    def open_document(self, file_name):
        ...
        """
        #import uno
        file_url = 'file:///%s' % (abspath(file_name))
        ...
        try:
            document = self.getDesktop().loadComponentFromURL(file_url, "_blank", 0, self.make_properties(load_properties))
        except Exception, ex:
    ...
    def save_document(self, document, file_name, close_document=True):
        ...
        try:
            ...
            raise OOHelperException(m_("Error saving file %s with OpenOffice: %s") % (file_name, ex))
        finally:
            if close_document:
                document.close(True)
    ...
    def replace_text_with_page_break(self, document):
        """
        Inserts page break into the document
        """
        #import uno
        search = document.createSearchDescriptor()
        search.SearchString = '${PAGE_BREAK}'
        ...
        while found:
            try:
                cursor = document.getText().createTextCursorByRange(found)
        ...
        if not self.ooServiceManager:
            ...
            while retry:
                ...
                try:
                    ...
                    resolver = OO_ServiceManager.createInstance("com.sun.star.bridge.UnoUrlResolver")
                    # Connect to the running OpenOffice.org and get its context.
                    oContext = resolver.resolve("uno:socket,host=%s,port=%s;urp;StarOffice.ComponentContext" % (self.cHost, self.cPort))
                    # Get the ServiceManager object
                    ooServiceManager = oContext.ServiceManager
                    return ooServiceManager
                except Exception, ex:
                    #
                    ...
                    if self.autostart_openoffice and retry:
                        #
                        # Try to start a headless OpenOffice server
                        #
                        OOPATH = 'C:\\Program Files (x86)\\OpenOffice.org 3\\program\\soffice.exe'
                        ooffice = None
                        if os.path.exists(OOPATH):
                                ooffice = OOPATH
                        if ooffice:
                            #self.log("Starting headless OpenOffice listener on port %s..." % cPort)
                            subprocess.call([ooffice, '-headless', '-nofirststartwizard', '-accept=socket,host=localhost,port=%s;urp;;StarOffice.Service' % self.cPort])
    ...
    def getCoreReflection(self):
        ...
        if not goCoreReflection:
            goCoreReflection = self.createUnoService( "com.sun.star.reflection.CoreReflection" )
    ...
    def createUnoStruct( self, cTypeName ):
        ...
        oStruct = OO_ServiceManager.Bridge_GetStruct("com.sun.star.beans.PropertyValue")
        return oStruct
    ...
    def make_properties(self, properties_dict):
            ...
            for key in properties_dict:
                prop = self.createUnoStruct("com.sun.star.beans.PropertyValue")
                prop.Name = key
