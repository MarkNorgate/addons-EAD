...
class cmc_assessment_report(osv.osv):
    _name = 'cmc.assessment.report'
    _description = 'cmc.assessment.report'
    _rec_name = 'name'
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None,
        ...
        if args:
            if isinstance(args, basestring):
                if args.rfind('id=')==0:
                    if args.split("=")[1] is not None:
                        cr.execute('SELECT s.id FROM cmc_assessment_report as s ' \
                        ...
                        args = [('id','in',ids)]
        ids = super(cmc_assessment_report, self).search(cr, uid, args, offset=offset, 
    ...
    _columns = {
        'name':fields.selection([('Standard Report','Standard Report'),
                                 ('Dvla Template','Dvla Template')],'Name'),
