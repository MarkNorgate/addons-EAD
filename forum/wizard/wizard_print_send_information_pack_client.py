...
class wizard_pending_assessment_client(wizard.interface):
    ...
    def _details(self, cr, uid, data, context):
        ...
        debug(prev_record)
        states = ['full_driving_assessment', 'adaptation_assessment', 'wheelchair_assessment', 'drive_from_wheelchair_assessment', 'passenger_assessment', 'car_assessment']
        for st in states:
            if prev_record.driving_assessment_type == st:
                debug(st)
                data['form'][st] = 'Y'
            else:
                data['form'][st] = ""
    ...
    states = {
        'init': {
            'actions': [_details],
            'result': {'type':'print',
                       'report':'pending.assessment.client',
                       'target':'new',
                       'state':'end'}
            },
    }

wizard_pending_assessment_client('pending.send.information.pack.client')
