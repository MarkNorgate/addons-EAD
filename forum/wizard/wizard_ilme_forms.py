...
report=False
class wizard_ilme_forms(wizard.interface):
    ...
    def _details(self, cr, uid, data, context):
        ...
        report='ilme.mobility.form'
        template_name=context.get('ilme_forms',False)
        event_start = False
        if template_name:
            report_id=pooler.get_pool(cr.dbname).get('ir.actions.report.xml').search(cr, uid, [("name", "=", "ILME AX Form")])
            report_rml="eMobility/Paperwork Template/"+template_name+".odt"
            report_type='oo-doc'
            debug(report_rml)
    ...
    states = {
        'init': {
            'actions': [_details],
            'result': {'type':'print', 'report':'ilme.mobility.form', 'target':'new', 'state':'end'}
            }
    }
wizard_ilme_forms('wizard_ilme_forms')
