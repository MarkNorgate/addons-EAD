...
def _my_equipment(self, cr, uid, data, context):
    query="select id from cmc_equipment where state='in_stock'"
    cr.execute(query)
    ids = cr.fetchall()
    debug(ids)
...
class wizard_cmc_equipment_in_stock(wizard.interface):
    states = {
        'init': {
            'actions': [],
...
wizard_cmc_equipment_in_stock('wizard_cmc_equipment_in_stock_stock')