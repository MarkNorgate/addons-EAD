...
class wizard_workshop_service_repair_jobsheet(wizard.interface):
...
#===============================================================================
        ...
        cr.execute('select id from cmc_appointments where state=\'active\' and workshop_id=%d ' % (int(data['ids'][0])))
        ...
        #=======================================================================
        data['form']['apmt_start_date']=(datetime.datetime.strptime(appt_record.apmnt_start_date,"%Y-%m-%d")).strftime("%d %B %Y") if appt_record else ""
        data['form']['apmt_start_time']=appt_record.start_time_hour+":"+appt_record.start_time_minutes if appt_record else ""
   ...
   states = {
        'init': {
            'actions': [_details],
            'result': {'type':'print', 'report':'workshop.service.repair.jobsheet', 'state':'end'}
            }
            
        
    }

wizard_workshop_service_repair_jobsheet('wizard_workshop_service_repair_jobsheet')
