...
class wizard_payment_refund_letter(wizard.interface):
    ...
    def _details(self, cr, uid, data, context):
        ...
        if prev_record.amount_debit:
            data['form']['refund']='%.2f' % prev_record.amount_debit
        else:
            data['form']['refund'] = '______________'
        pooler.get_pool(cr.dbname).get('cmc.assessment.communication').create(cr, uid, {
                 ...
                 'message':'Refund Letter for Client Printed'
                })
        return data['form']
    ...
    states = {
        'init': {
            'actions': [_details],
            'result': {'type':'print', 'report':'payment.refund.letter', 'state':'end'}
            }
            
        
    }

wizard_payment_refund_letter('wizard.payment.refund.letter.client')
